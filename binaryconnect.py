...
class BC():
    ...
    def save_params(self):
        # This loop goes through the list of target modules,
        # and saves the corresponding weights into the list of saved_parameters
        for index in range(self.num_of_params):
            self.saved_params[index].copy_(self.target_modules[index].data)

    def binarisation(self):
        # (1) Save the current full precision parameters using the
        # save_params method
        self.save_params()
        ### (2) Binarize the weights in the model, by iterating through the
        # list of target modules and overwrite the values with their binary version
        for i in range(self.num_of_params):
            self.target_modules[i].data.copy_(
                where(
                    self.target_modules[i].data >= 0,
                    1,
                    -1,
                )
            )
    ...
    def clip(self):
        ...
        for i in range(self.num_of_params):
            self.target_modules[i].data.copy_(F.hardtanh(self.target_modules[i]))
            self.saved_params[i].data.copy_(F.hardtanh(self.saved_params[i]))
